...
export function BatchResults({ results }: BatchResultsProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Summary Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-lg border border-zinc-800 bg-zinc-900/50">
        <div className="flex items-center gap-4 text-sm font-medium">
          <span className="text-zinc-200">{results.length} Processed</span>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="outline" size="sm" onClick={handleExportCsv} className="border-zinc-700 h-8 gap-2">
      ...
      <div className="space-y-2">
        {results.map((result) => (
          <div key={result.id} className="border border-zinc-800 rounded-lg bg-zinc-950 overflow-hidden transition-all">
            {/* Row header */}
            <div
              className="flex items-center justify-between p-3 cursor-pointer hover:bg-zinc-900/50 transition-colors"
